  ...
  <div class="flex flex-row items-end gap-1">
    <ul class="ms-2 w-full grow space-y-1">
      <li
        ...
        :title="`${localizeJobCategory(job.job_category, t)} - ${localizeJobStatus(job.status, t)}`"
      >
        <UIcon
          ...
          :title="job.job_id"
        />
        <span v-else>{{ localizeJobCategory(job.job_category, t) }}</span>
        <div class="flex justify-between">
          ...
          <div class="flex gap-2 items-center w-full" v-else-if="job.status === 'failed'">
            ...
            <span class="text-error">{{ localizeJobStatus(job.status, t) }}</span>
            <span class="ms-2">{{ job.error }}</span>
          </div>
          <span v-else>{{ localizeJobStatus(job.status, t) }} </span>
        ...
        >
          <div v-for="[tkey, t] in Object.entries(job.progress_tasks)" :key="tkey" class="w-full">
            {{ localizeJobDescription(t.description, $t) }}
