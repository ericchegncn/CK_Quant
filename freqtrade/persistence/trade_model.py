...
class LocalTrade:
    ...
    def to_json(self, minified: bool = False, include_orders: bool = True) -> dict[str, Any]:
        ...
        # include_orders=False 为本 fork 的轻量模式（WebUI/回测用）：不加载订单集合。
        # 下游的 filled_*/open_* 列表随之全为空，因此 nr_of_successful_* / has_open_orders
        # / date_entry_fill_utc 自动退化为 0/False/None，与旧实现语义一致。
        # 上游 2026.8 移除了该门控 —— 不要跟随，否则轻量模式会重新加载全部订单。
