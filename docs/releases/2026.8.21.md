...
3. 异常冒泡到主循环（`worker._worker`）被当作致命错误，进程退出；容器重启后立刻重新拉取全部持仓的
   资金费率/止损，再次撞上限流 → 形成"崩溃—重启—再崩溃"的死循环，且每次重启都在给限流加码。
...
**运维建议**：`internals.process_throttle_secs` 由 5 秒调至 30 秒可把 API 压力降至约 1/6。
多个机器人共用同一出口 IP 时尤其建议调高（本机已按 30 秒运行）。

## Docker 镜像

- 标准镜像发布为 `2026.8.21`、`stable` 和 `latest`。

## Android APK

- `CK-Quant-Android-0.1.15-debug.apk`（versionCode 16）。
